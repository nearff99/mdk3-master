...
// New line removed
    ...
    if (len > max_len) {
        ...
        len = strlen(ssid);
        if (showssidwarn2) {
            printf("\nWARNING! Truncating overlenght SSID to 255 bytes!\n");
            showssidwarn2 = 0;
        }
    } else {
...
void wids_sniffer()
{
    ...
    while (1) {
        ...
        case 0x80: //Beacon frame
            ...
            wids_stats.aps++;
            if (zc_exploit) {
                printf("\nFound foreign AP: %02X:%02X:%02X:%02X:%02X:%02X on channel %d           \n", bi.bssid[0], bi.bssid[1], bi.bssid[2], bi.bssid[3], bi.bssid[4], bi.bssid[5], bi.channel);
            } else {
                printf("\nFound AP: %02X:%02X:%02X:%02X:%02X:%02X on channel %d           \n", bi.bssid[0], bi.bssid[1], bi.bssid[2], bi.bssid[3], bi.bssid[4], bi.bssid[5], bi.channel);
            }
        break;
...
struct pckt wids_machine()
{
    ...
    if (! init_wids) {
        ...
        for (t=0; t<10; t++) {
            sleep(1);
            printf("\nAPs found: %d   Clients found: %d", wids_stats.aps, wids_stats.clients);
        }
        ...
        while (!init_clientlist) {
            printf("\nNo clients found yet. If it doesn't start, maybe you need to fake additional clients with -c\n");
            sleep(5);
        }
        init_wids = 1;
    }
...
// Checking if Authentication DoS is successful
    ...
    if (status == 0) //Nothing heard from AP so far
    {
        if (resp) //AP responding for the first time
        {
            ...
            auths[pos][1]++;
            printf("\nAP %02X:%02X:%02X:%02X:%02X:%02X is responding!           \n", ap[0], ap[1], ap[2], ap[3], ap[4], ap[5]);
        }
        return;
    }
    if (status == 1) //Ap is known to respond
    {
        if (resp) //Ap keeps responding
        {
            ...
            {
                printf("\nAP %02X:%02X:%02X:%02X:%02X:%02X seems to be INVULNERABLE!      \n", ap[0], ap[1], ap[2], ap[3], ap[4], ap[5]);
    ...
    if (status == 2) //Ap stopped responding
    {
        if (resp) //False alarm, AP responding again
        {
            ...
            if (auths[pos][2] > 50) //50 responses missing => Another one bites the dust!
            {
                ...
                printf("\nAP %02X:%02X:%02X:%02X:%02X:%02X seems to be VULNERABLE and may be frozen!\n", ap[0], ap[1], ap[2], ap[3], ap[4], ap[5]);
    ...
    if (status == 3) //AP under test
    {
        if (resp) //AP is back in action!
        {
            ...
            auths[pos][1] = 0;
            auths[pos][2] = 0;
            printf("\nAP %02X:%02X:%02X:%02X:%02X:%02X has returned to functionality!     \n", ap[0], ap[1], ap[2], ap[3], ap[4], ap[5]);
        }
        return;
    }
}
...
//Removed '+1' since it always added a strange extra character to the output (?).
    ...
    printf("\nCurrent MAC: %02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
...
// Print some information while in Authentication DoS mode
    ...
    printf("\nConnecting Client: %02X:%02X:%02X:%02X:%02X:%02X", fc[0], fc[1], fc[2], fc[3], fc[4], fc[5]);
...
void print_probe_stats(int responses, int sent)
{
    ...
    printf("\nAP responded on %d of %d probes (%d percent)                  \n", responses, sent, perc);
}
...
// Print some information while in Deauthentication DoS mode
    ...
    printf("\nDisconnecting between: %02X:%02X:%02X:%02X:%02X:%02X", fc[0], fc[1], fc[2], fc[3], fc[4], fc[5]);
...
{
    printf("\nClients: Created: %4d   Authenticated: %4d   Associated: %4d   Got Kicked: %4d\n",
...
{
    printf("\nAPs found: %d   Clients found: %d   Completed Auth-Cycles: %d   Caught Deauths: %d\n",
...
void print_mac_bruteforcer_stats(struct pckt packet)
{
    ...
    printf("\nTrying MAC %02X:%02X:%02X:%02X:%02X:%02X with %8.4f ms timeout at %3d MACs per second and %d retries\n",
...
void print_wpa_downgrade_stats()
{
    ...
    printf("\nDeauth cycles: %4d  802.1x authentication packets: %4d  WEP/Unencrypted packets: %4d  Beacons/sec: %3d\n", wpad_cycles, wpad_auth, wpad_wep, wpad_beacons);
    if (wpad_beacons == 0) {
...
int mdk_parser(int argc, char *argv[])
{
    ...
    int pps = 500;                 // Packet sending rate
    int t = 0;
        ...
        for (t=3; t<argc; t++)
        {
            if (! strcmp(argv[t], "-b"))
            {
                if(argc<=7){
                        ...
                        \n a channel (-c), a target-mac (-t) and a character-set:\n \
                        \n all printable (a)\n \
                        \n lower case (l)\n \
                        \n upper case (u)\n \
                        \n numbers (n)\n \
...
statshortcut:
        ...
        if( time( NULL ) - t_prev >= 1 )
        {
            ...
            printf ("\nPackets sent: %6d - Speed: %4d packets/sec", nb_sent, nb_sent_ps);
            fflush(stdout);
            nb_sent_ps=0;
            resps=0;
            total_time++;
